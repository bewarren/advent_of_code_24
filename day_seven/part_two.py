...
def recurive_find_value(ans, value, values):
    if len(values) == 1:
        return (ans == value*values[0]) or (ans == value + values[0]) or (ans == int(str(value)+ str(values[0])))
    else:
        return recurive_find_value(ans, values[0] + value, values[1:]) or recurive_find_value(ans, values[0] * value, values[1:]) or recurive_find_value(ans, int(str(value) + str(values[0])), values[1:])
